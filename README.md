# AvtoFotaNew
![ESP32](https://img.shields.io/badge/ESP32-supported-blue.svg)
![ESP8266](https://img.shields.io/badge/ESP8266-supported-blue.svg)
![Arduino](https://img.shields.io/badge/Arduino-IDE-00979D.svg)
![PlatformIO](https://img.shields.io/badge/PlatformIO-supported-orange.svg)

![OTA](https://img.shields.io/badge/OTA-HTTP%2FHTTPS-success.svg)
![Progress](https://img.shields.io/badge/OTA-Progress%200--100%25-brightgreen.svg)
![Watchdog](https://img.shields.io/badge/Watchdog-safe-green.svg)

![MQTT](https://img.shields.io/badge/MQTT-ready-green.svg)
![Callback](https://img.shields.io/badge/Callback-supported-blueviolet.svg)

![AvtoFotaNew](https://img.shields.io/badge/AvtoFotaNew-v1.0.0-yellow.svg)
![License](https://img.shields.io/badge/License-MIT-orange.svg)


![GitHub release](https://img.shields.io/github/v/release/eu1abg/AvtoFotaNew)
![GitHub stars](https://img.shields.io/github/stars/eu1abg/AvtoFotaNew?style=social)
![GitHub issues](https://img.shields.io/github/issues/eu1abg/AvtoFotaNew)

**Лёгкая и надёжная OTA-библиотека для ESP32 и ESP8266**  
с поддержкой **блокирующего и неблокирующего обновления**,  
**прогресса в реальном времени**, **Watchdog-safe** и полной свободой интеграции
(MQTT, Serial, дисплеи, Home Assistant и т.д.).

---

## 🚀 Возможности

- ✅ ESP32 и ESP8266
- ✅ HTTP / HTTPS
- ✅ Прошивки > 1 MB
- ✅ Блокирующий OTA (`updateNOW`)
- ✅ Неблокирующий OTA (`update` в `loop`)
- ✅ Прогресс 0–100%
- ✅ Callback прогресса
- ✅ Watchdog-safe
- ✅ MQTT / Serial / Display — без изменений библиотеки
- ✅ Arduino IDE и PlatformIO

---

## 📁 Структура
```Text
AvtoFotaNew/
├─ AvtoFotaNew.h
├─ AvtoFotaNew.cpp
├─ examples/
│ ├─ BlockUpdate/
│ ├─ LoopUpdate/
│ └─ MqttOtaProgress/
├─ library.properties
├─ library.json
└─ README.md
```


## 📦 Установка

### Arduino IDE
1. Скопируйте папку `AvtoFotaNew` в: Documents/Arduino/libraries/
2. Перезапустите Arduino IDE

### PlatformIO
Поместите библиотеку в папку `lib/` проекта  
или подключите через `lib_deps`.

---

## ⚙️ Обязательные настройки платы

### ESP32
- **Partition Scheme** → `Default with OTA`

### ESP8266
- **Flash Size** → `4MB (OTA)`
- Иначе OTA работать не будет

---

## 🔗 JSON-манифест

Пример файла `firmware.json`:

```json
{
"version": "1.1.0",
"notes": "Bug fixes and stability improvements",
"bin": "https://yourserver.com/firmware.bin"
}
```
## 🧠 Принцип работы

1. Устройство загружает JSON-манифест

2. Сравнивает версии

3. Если версия новая — скачивает .bin

4. Обновляет прошивку

5. Передаёт прогресс (callback)

6. Перезагружается (опционально)

   
## 🧠 Основные понятия

- Блокирующий OTA — выполнение останавливается до конца обновления

- Неблокирующий OTA — вызывается из loop(), остальной код продолжает работать

- Прогресс — доступен в обоих режимах

- MQTT / Serial / Display — подключаются через callback, библиотека остаётся чистой

## 📌 API библиотеки
---
### 🔹 Конструктор
```cpp
AvtoFotaNew(const char* version);
``` 
Создаёт объект OTA.
`version ` — текущая версия прошивки

### 🔹 setManifestURL()
```cpp
void setManifestURL(const char* url);
```
Устанавливает URL JSON-манифеста.

### 🔹 setDebug()

```cpp
void setDebug(bool enable);
```
Включает / отключает вывод отладочной информации в Serial.

### 🔹 setProgressCallback()

```cpp
void setProgressCallback(std::function<void(uint8_t)> callback);
```
Callback прогресса OTA (0–100%).

- Используется для:

- erial

- MQTT

- дисплеев

- логирования

### 🔹 getchekupdate()

```cpp
bool getchekupdate();
```
Проверяет наличие обновления.
Возвращает:
`true` — обновление доступно
`false` — обновлений нет или ошибка

### 🔹 getupdate()
```cpp
bool getupdate(String &version, String &notes);
```
Возвращает данные доступного обновления:
`version` — новая версия
`notes` — описание изменений

### 🔹 getpercentupdate()
```cpp
uint8_t getpercentupdate() const;
```
Возвращает текущий процент загрузки OTA.
 📌 В блокирующем режиме используйте callback.

### 🔹 getVER()

```cpp
String getVER() const;
```
Возвращает текущую версию прошивки.

---

## 🔄 OTA-обновление

### 🔒 Блокирующий режим
```cpp
bool updateNOW(bool autorestart = true);
```

-Полностью блокирует выполнение до конца OTA

-Прогресс передаётся через callback

`autorestart = true` — автоматическая перезагрузка

### 🔁 Неблокирующий режим
```cpp
bool update(bool autorestart = true);
```
Вызывается из `loop()`
Не блокирует основной код
Возвращает `true`, когда OTA завершено

### 📡 MQTT-пример (прогресс OTA)
```cpp
fota.setProgressCallback([](uint8_t p) {
  mqtt.publish("device/esp32/fota/progress", String(p).c_str());
});
```

📌 Библиотека не содержит MQTT — это сделано намеренно
для минимального размера и максимальной гибкости.

| Режим OTA     | Прогресс через getpercentupdate() | Callback |
| ------------- | ----------------------------------- | -------- |
| Блокирующий   | ❌                                  | ✅        |
| Неблокирующий | ✅                                  | ✅        |

## 🧩 Архитектурный принцип

1. Библиотека отвечает только за OTA
2. MQTT, Serial, дисплеи и логика — задача приложения

Это делает AvtoFotaNew:

- лёгкой

- стабильной

- расширяемой
---
## 👨‍💻 Автор

### Aliaksandr Labada
GitHub: https://github.com/eu1abg

## 🏁 Итог

AvtoFotaNew — это готовое решение для реальных проектов:

-просто подключить

-легко расширить

-безопасно обновлять
